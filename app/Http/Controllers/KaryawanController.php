...
class KaryawanController extends Controller
{
    public function index(Request $request)
    {
        ...
        $query->select('karyawans.*','dept_name');
        $query->join('departments', 'karyawans.dept_code', '=', 'departments.dept_code');
        $query->orderBy('nama_lengkap');
        if (!empty($request->employee_name)) {
            $query->where('nama_lengkap','like','%'.$request->employee_name.'%');
        }

        if (!empty($request->dept_code)) {
            $query->where('karyawans.dept_code', $request->dept_code);
        }
